  ...
  if (!data?.food) {
    return (
      <div className="max-w-xl">
        ...
        <p className="text-forest font-semibold mb-1">&ldquo;{name}&rdquo; wasn&apos;t found in the database.</p>
        <p className="text-sm text-gray-500">Try a simpler name (e.g. &ldquo;chard&rdquo; instead of &ldquo;Swiss chard&rdquo;) or check the spelling.</p>
      </div>
    )
  }
  ...
  return (
    <div>
        ...
        <table className="w-full text-sm border border-gray-100 rounded-xl overflow-hidden shadow-sm">
          <thead className="bg-mint text-left text-xs text-fern font-semibold">
            <tr>
              ...
              <th className="px-3 py-2.5 text-right">Per 100 g</th>
              <th className="px-3 py-2.5 text-right">% RDI</th>
              <th className="px-3 py-2.5">Best prep</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {nutrients.map(row => (
              <tr key={row.nutrient.id} className="align-top hover:bg-gray-50 transition-colors">
                ...
                <td className="px-3 py-2.5 text-xs text-gray-600 max-w-[200px]">{row.best_prep_method}</td>
                <td className="px-3 py-2.5 text-xs text-fern">
                  {row.enhancers.length > 0 ? row.enhancers.map(e => e.compound).join(', ') : <span className="text-gray-300">—</span>}
                </td>
                <td className="px-3 py-2.5 text-xs text-coral">
                  {row.inhibitors.length > 0 ? row.inhibitors.map(e => e.compound).join(', ') : <span className="text-gray-300">—</span>}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="md:hidden space-y-2">
        {nutrients.map(row => (
          <div key={row.nutrient.id} className="border border-gray-100 rounded-xl px-4 py-3 bg-white shadow-sm">
            <div className="flex items-start justify-between gap-2">
              <div>
                <span className="font-semibold text-sm text-forest">{row.nutrient.name}</span>
                {row.nutrient.vitamer_form && (
                  <span className="ml-1 text-xs text-gray-400">{row.nutrient.vitamer_form}</span>
                )}
              </div>
              {row.pct_rdi != null && (
                <span className={`text-xs font-semibold shrink-0 ${row.pct_rdi >= 25 ? 'text-forest' : row.pct_rdi >= 10 ? 'text-fern' : 'text-gray-500'}`}>
                  {row.pct_rdi}% RDI
                </span>
              )}
            </div>
            ...
            {(row.enhancers.length > 0 || row.inhibitors.length > 0) && (
              <button
                ...
                className="mt-2 text-xs text-fern underline"
              >
